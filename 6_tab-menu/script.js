...
function test1() {
  ...
  // hint: data-user-id는 dataset.userId로 접근 (카멜 케이스)
  // console.log("유저 ID:", );
  ...
  // hint: data-joined-at은 dataset.joinedAt으로 접근
  // console.log("가입일:", );
...
function test3() {
  ...
  // hint: dataset.price는 문자열이므로 Number()로 변환 필요

  // products.forEach((product) => {
  //   const price = (total += Number(product.dataset.price));
  //   console.log(product.dataset.productId, ":", price, "원");
  // });
  let total = [...products].reduce(
    (acc, product) =>
      // acc += Number(product.dataset.price);
      acc + Number(product.dataset.price),
    0,
  );

  console.log("총 합계:", total, "원");
  // 예상 출력: 총 합계: 1334000 원
}
...
function test5() {
  ...
  // hint: querySelectorAll('[data-status="error"]')
  // const errorItems =
  const errorItems = document.querySelectorAll(
    "#filter-list [data-status='error']",
  );

  // TODO: 선택한 요소들을 forEach로 순회하며 textContent를 콘솔에 출력하세요
  errorItems.forEach((errorItem) => console.log(`${errorItem.textContent}`));
